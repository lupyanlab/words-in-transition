...
response_counts <- imitation_matches %>%
  count(imitation_id, survey_type)

imitation_id_decr <- response_counts %>%
  group_by(imitation_id) %>%
  summarize(total_n = sum(n)) %>%
  arrange(desc(total_n)) %>%
  .$imitation_id
response_counts$imitation_id_decr <- factor(response_counts$imitation_id,
                                            levels = imitation_id_decr)

ggplot(response_counts, aes(x = imitation_id_decr, y = n)) +
...
TODO: Add in results presented at EvoLang
