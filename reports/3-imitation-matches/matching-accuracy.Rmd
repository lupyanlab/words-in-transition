...
# Extract the fixed effects intercept
mod_fixed_effects <- tidy(imitation_matches_mod, effects="fixed") %>%
...
seed_category_preds <- expand.grid(
  ...
  left_join(mod_fixed_effects)
  mutate(
    offset = fixed_intercept +
             (seed_intercept + seed_slope * generation_1) + 
...
seed_means %<>%
  ...
  mutate(
    is_correct_adj = exp(offset)/(1 - exp(offset))
  )
...
```{r}
imitation_matches$predicted <- predict(imitation_matches_mod, type = "response")

ggplot(imitation_matches) +
  aes(generation_1, predicted, color = survey_type) +
