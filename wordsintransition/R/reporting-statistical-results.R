...
glmer_mod_results <- function(glmer_mod, param, odds = FALSE) {
  ...
  if (odds == TRUE) {
    formatted = sprintf("_b_ = %.2f (%.2f) log-odds, odds = %.2f, _z_ = %.2f, _p_ = %.2f",
                        results$estimate, results$std.error, results$odds, results$statistic, results$p.value)
  } else {
    formatted = sprintf("_b_ = %.2f (%.2f) log-odds, _z_ = %.2f, _p_ = %.2f",
                        results$estimate, results$std.error, results$statistic, results$p.value)
  }
  formatted
}
